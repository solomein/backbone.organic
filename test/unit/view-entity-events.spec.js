describe('view entity events', function () {
    ...
    describe('when a view has model event config with a specified handler method that doesnt exist', function () {
        ...
        it('should error when method doesnt exist', function () {
            expect(this.getBadViewInstance)
    ...
    describe('when LayoutView bound to modelEvent replaces region with new view', function () {
        beforeEach(function () {
            ...
            this.layout = new this.Layout({model: this.model});
            this.viewOne = new this.View({model: this.model});
            this.viewTwo = new this.View({model: this.model});

            this.layout.render();
            this.layout.regions.child.show(this.viewOne);
            this.layout.regions.child.show(this.viewTwo);

            this.model.trigger('baz');
        });

        it('should leave the layout\'s modelEvent binded', function () {
            expect(this.fooStub).to.have.been.calledOnce;
        });
