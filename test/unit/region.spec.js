describe('Organic.Region', function () {
    'use strict';
    ...
    describe('when passing an el DOM reference in directly', function () {
        beforeEach(function () {
            ...
            this.customRegion = new (Organic.Region.extend({
                el: this.el
            }))();

            this.optionRegion = new Organic.Region({
                el: this.el
            });

            this.optionRegionJquery = new Organic.Region({
                el: $(this.el)
            });
        });
    ...
    describe('when creating a new region and the "el" does not exist in DOM', function () {
        beforeEach(function () {
            this.MyRegion = Organic.Region.extend({
                el: '#not-existed-region'
            });

            this.MyView = Organic.BaseView.extend({
                render: function () {
    ...
    describe('when showing an initial view', function () {
        beforeEach(function () {
            ...
            this.attachHtmlSpy = this.sinon.spy(this.myRegion, 'attachViewHtml');
    ...
    describe('when showing nested views', function () {
        beforeEach(function () {
            ...
            this.LayoutView = Organic.Layout.extend({
                regionsDecl: {
                ...
                onBeforeShow: function () {
                    this.regions.subRegion.show(new self.SubView());
                }
            });

            this.SubView = Organic.View.extend({
                render: function () {
                    $(this.el).html('some content');
                },

                initialize: function () {
                    self.innerRegionBeforeShowSpy = self.sinon.spy();
                    self.innerRegionShowSpy = self.sinon.spy();
                    this.on('before:show', self.innerRegionBeforeShowSpy);
                    this.on('show', self.innerRegionShowSpy);
                }
            });
    ...
    describe('when a view is already shown and showing the same one with a forceShow flag', function () {
        beforeEach(function () {
            ...
            this.sinon.spy(this.view, 'destroy');
            this.sinon.spy(this.myRegion, 'attachViewHtml');
            this.sinon.spy(this.view, 'render');
            this.myRegion.show(this.view, {
                forceShow: true
            });
        ...
        it('should call "attachHtml" on the view', function () {
            expect(this.myRegion.attachViewHtml).to.have.been.calledWith(this.view);
        });
    ...
    describe('when a view is already shown but destroyed externally', function () {
        beforeEach(function () {
            ...
            this.MyView = Organic.View.extend({
    ...
    describe('when creating a region instance with an initialize method', function () {
        beforeEach(function () {
            ...
            this.Region = Organic.Region.extend({
                el: '#foo',
                initialize: function () {}
            });
    ...
    describe('when removing a region', function () {
        beforeEach(function () {
            ...
            this.layout.addRegions({
                MyRegion: '#region',
                anotherRegion: '#region2'
            });

            this.region = this.layout.regions.MyRegion;
            this.sinon.spy(this.region, 'empty');

            this.layout.removeRegion('MyRegion');
        });

        it('should be removed from the app', function () {
            expect(this.layout.regions.MyRegion).to.be.undefined;
        });
    ...
    describe('when destroying a view in a region', function () {
        beforeEach(function () {
            ...
            this.View = Organic.BaseView.extend({
                template: _.template('')
            });
